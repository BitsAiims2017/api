...
exports.get_user = (username, done) => {
    ...
    User.findOne({username}, 'name username role meta joined -_id', (err, user) => {
        if (err) {
    ...
    User.findOne({username}, (err, user) => {
        if (err || !user) {
            ...
            for (var prop in data) {
                if (data[prop] != '' && user[prop]) { user[prop] = data[prop]; }
            }
            user.save((err) => {
                if (err) {
                  console.log(err);
...
exports.get_all_users = (done) => {
    User.find({}, 'name username role meta joined -_id', (err, user) => {
        if (err) {
